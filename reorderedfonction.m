...
function reorderedfonction(lien, out)

    % Importation des données depuis Brainstorm
    sExport = in_bst_data(lien);

    % Load channel file
    channel_file = bst_get('ChannelFileForStudy', lien);
    ...
    newFile = struct();
    newFile.Fs = 128;
    newFile.data = data_reordered;
    newFile.channels = channels_reordered ;

    %save('/NAS/home/auristre/Documents/software/SpikeNet1/Data/raw/data_tutorial_preprocessed_reordered.mat', '-struct', 'newFile')
